...
def train(config_path,
    ...
    valid_input_cfg = config.valid_input_reader
    ...
    if loss_scale < 0:
        loss_scale = "dynamic"
    if train_cfg.enable_mixed_precision:
        max_num_voxels = train_input_cfg.preprocess.max_number_of_voxels * train_input_cfg.batch_size
    ...
    train_dataset = input_reader_builder.build(
        ...
        net=net,
        multi_gpu=multi_gpu)
    valid_dataset = input_reader_builder.build(
        valid_input_cfg,
        model_cfg,
        training=False,
        voxel_generator=voxel_generator,
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size=train_input_cfg.batch_size * num_gpu,
        sampler=train_dataset.sampler(shuffle=True),
    ...
    valid_dataloader = torch.utils.data.DataLoader(
        valid_dataset,
        batch_size=valid_input_cfg.batch_size, # only support multi-gpu train
        sampler=valid_dataset.sampler(shuffle=False),
    ...
    eval_dataloader = torch.utils.data.DataLoader(
        ...
        sampler=eval_dataset.sampler(shuffle=False),
    ...
    try:
        while True:
            ...
            for example in train_dataloader:
                ...
                if (global_step % steps_per_save == 0) or (step == total_step - 1):
                    torchplus.train.save_models(model_dir, [net, amp_optimizer],
                                                net.get_global_step())

                if global_step % steps_per_eval == 0:
                    net.eval()
                    result_path_step = Path(result_path) / f"step_{net.get_global_step()}"
                    result_path_step.mkdir(parents=True, exist_ok=True)


                    model_logging.log_text("#################################",
                                        global_step)
                    model_logging.log_text("# EVAL: VALID SET", global_step)
                    model_logging.log_text("#################################",
                                        global_step)
                    ...
                    net.clear_timer()
                    prog_bar.start((len(valid_dataset) + valid_input_cfg.batch_size - 1)
                                // valid_input_cfg.batch_size)
                    for example in iter(valid_dataloader):
                        example = example_convert_to_torch(example, float_dtype)
                        detections += net(example)
                        prog_bar.print_bar()

                    sec_per_ex = len(valid_dataset) / (time.time() - t)
                    model_logging.log_text(
                        f'generate label finished({sec_per_ex:.2f}/s). start eval:',
                        global_step)
                    result_dict = valid_dataset.evaluation(detections, str(result_path_step), prefix="valid")
                    for k, v in result_dict["results"].items():
                        model_logging.log_text("Evaluation VALID {}".format(k), global_step)
                        model_logging.log_text(v, global_step)
                    model_logging.log_metrics(result_dict["detail"], global_step)
                    with open(result_path_step / "result_valid.pkl", 'wb') as f:
                        pickle.dump(detections, f)
                    

                    model_logging.log_text("#################################",
                                        global_step)
                    model_logging.log_text("# EVAL: TEST SET", global_step)
                    model_logging.log_text("#################################",
                                        global_step)
                    model_logging.log_text("Generate output labels...", global_step)
                    t = time.time()
                    ...
                    model_logging.log_text(
                        f'generate label finished({sec_per_ex:.2f}/s). start eval:',
                        global_step)
                    result_dict = eval_dataset.evaluation(detections, str(result_path_step), prefix="test")
                    for k, v in result_dict["results"].items():
                        model_logging.log_text("Evaluation {}".format(k), global_step)
                        model_logging.log_text(v, global_step)
                    model_logging.log_metrics(result_dict["detail"], global_step)
                    with open(result_path_step / "result_test.pkl", 'wb') as f:
                        pickle.dump(detections, f)
...
def evaluate(config_path,
    ...
    with open(result_path_step / "result.pkl", 'wb') as f:
        pickle.dump(detections, f)
    result_dict = eval_dataset.evaluation(detections,
                                          str(result_path_step),
                                          prefix="test")
    if result_dict is not None:
        with open(result_path_step / "metrics.txt", "w") as f:
            for k, v in result_dict["results"].items():
                print("Evaluation {}".format(k))
                print("Evaluation {}".format(k), file=f)
                print(v)
                print(v, file=f)
