...
from second.data import vkitti_common as vkitti
...
@register_dataset
class VirtualKittiDataset(Dataset):
    ...
    def __init__(self,
        ...
        self._vkitti_infos = infos

        print("remain number of infos:", len(self._vkitti_infos))
    ...
    def convert_detection_to_kitti_annos(self, detection):
        ...
        for i in range(len(detection)):
            ...
            anno = vkitti.get_start_result_anno()
            num_example = 0
            ...
            if num_example != 0:
                ...
                annos.append(vkitti.empty_result_anno())
    ...
    def get_sensor_data(self, query):
        ...
        np_depth_image = np.array(Image.open(depth_path))  # (W, H) dtype=np.int32
        # Convert centimeters (int32) to meters.
        np_depth_image = np_depth_image.astype(np.float32) / 100  # (W, H) dtype=np.float32
        res["depth"] = {
            "type": "depth_map",
        ...
        R0_rect = np.eye(4, dtype=np.float32) # there is no rectification in vkitti
...
def create_vkitti_info_file(data_path, save_path=None, relative_path=True):
    imageset_folder = Path(__file__).resolve().parent / "VKittiImageSets"
    ...
    def gen_vkitti_infos(img_ids, filename):
        print("Generate info. this may take several minutes.")
        vkitti_infos = vkitti.get_vkitti_image_info(
            data_path,
            depth=True,
            image_ids=img_ids,
            relative_path=relative_path)
        # _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path)
        print(f"VKitti info file is saved to {filename}")
        with open(filename, 'wb') as f:
            pickle.dump(vkitti_infos, f)
    
    img_ids = _read_imageset_file(str(imageset_folder / "train.txt"))
    gen_vkitti_infos(img_ids, save_path / 'vkitti_infos_train.pkl')
    
    img_ids = _read_imageset_file(str(imageset_folder / "test.txt"))
    gen_vkitti_infos(img_ids, save_path / 'vkitti_infos_test.pkl')

    img_ids = _read_imageset_file(str(imageset_folder / "test_clone_only.txt"))
    gen_vkitti_infos(img_ids, save_path / 'vkitti_infos_test_clone_only.pkl')

    img_ids = _read_imageset_file(str(imageset_folder / "train_random_subset.txt"))
    gen_vkitti_infos(img_ids, save_path / 'vkitti_infos_train_random_subset.pkl')
