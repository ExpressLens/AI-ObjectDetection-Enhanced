...
class KittiDataset(Dataset):
    ...
    def getitem_from_sensor_data(self, input_dict):
        ...
        if self._prep_func_data_aug is not None:
            points, gt_dict = self._prep_func_data_aug(points, gt_dict)
        ...
        if self._prep_func_add_targets is not None:
            example = self._prep_func_add_targets(example, gt_dict, calib)
    ...
    def get_sensor_data(self, query):
        ...
        create_sb_lidar = False
        idx = query
        if isinstance(query, dict):
            read_image = "cam" in query
            create_depth_map = "depth" in query
            create_sb_lidar = "sb_lidar" in query
        ...
        if create_sb_lidar:
            ...
            sb_mask = (points[:, 2] > self._SB_LIDAR_RANGE[0]) & \
                    (points[:, 2] < self._SB_LIDAR_RANGE[1])
            sb_points = points[sb_mask, :3]  # (N, 3)
            res["sb_lidar"]["points"] = sb_points
        ...
        if create_depth_map:
            Trv2lccam = self.lc_device.TRANSFORMS['wTc']  # extrinsics matrix
            points_lccam = points @ Trv2lccam.T
            depth_map = cam_transforms.point_cloud_to_depth_map(
                points_lccam,
                self.lc_device.CAMERA_PARAMS['matrix'],
                self.lc_device.CAMERA_PARAMS['width'],
                self.lc_device.CAMERA_PARAMS['height'])
            res["depth"] = {
                "type": "depth_map",
                "image": depth_map
            }
